...
Static site with no framework, no package manager, and no bundler. Three HTML pages, two JS files, one CSS file.
...
`index.html` loads three.js via a CDN import map (jsdelivr, pinned). `phosphor.js` prefers the WebGL [GPGPU birds + GLTF](https://threejs.org/examples/webgl_gpgpu_birds_gltf.html) path (Parrot mesh, baked wing morphs, mouse scatter). If that fails it falls back to the [compute birds](https://threejs.org/examples/webgpu_compute_birds.html) triangles, then a CPU instanced flock. `prefers-reduced-motion` freezes the flock; the loop pauses when the tab is hidden.
